...
try {
    "[$timestamp] [START] COMPLETE RESTART PROTOCOL STARTED" | Out-File $logFile -Encoding UTF8 -Append
    
    # STEP 1: SAVE ALL MEMORY FIRST
    "[$timestamp] Step 1/3: Saving memory..." | Out-File $logFile -Encoding UTF8 -Append
    $memorySave = Start-Process powershell -ArgumentList "-NonInteractive -WindowStyle Hidden -File C:\Dev\current-memory-save-restart.ps1" -NoNewWindow -Wait -PassThru
    if ($memorySave.ExitCode -eq 0) {
        "[$timestamp] [OK] Memory saved successfully" | Out-File $logFile -Encoding UTF8 -Append
    } else {
        "[$timestamp] [WARN] Memory save completed with exit code $($memorySave.ExitCode)" | Out-File $logFile -Encoding UTF8 -Append
    }
    ...
    try {
        ...
        if (Test-Path $gatewayCmd) {
            ...
            try {
                ...
                "[$timestamp2] [OK] Restart completed successfully - gateway responding" | Out-File $logFile -Encoding UTF8 -Append
                "[$timestamp2] [MEMORY] Memory preserved - Burgundy will resume with full context" | Out-File $logFile -Encoding UTF8 -Append
                
                # AUTO-WAKE IMPLEMENTATION (Added March 29, 2026)
                "[$timestamp2] [AUTO-WAKE] Sending wake event..." | Out-File $logFile -Encoding UTF8 -Append
                Start-Sleep -Seconds 5  # Wait for gateway to be ready
                try {
                    & openclaw cron wake --text "Gateway restarted - resuming operation" --mode now
                    "[$timestamp2] [AUTO-WAKE] Wake event sent successfully" | Out-File $logFile -Encoding UTF8 -Append
                }
                catch {
                    "[$timestamp2] [AUTO-WAKE] Wake event failed: $($_.Exception.Message)" | Out-File $logFile -Encoding UTF8 -Append
                }
            }
            catch {
                "[$timestamp2] [WARN] Gateway started but not responding yet" | Out-File $logFile -Encoding UTF8 -Append
            }
        } else {
            "[$timestamp] [ERROR] Gateway CMD file not found at: $gatewayCmd" | Out-File $logFile -Encoding UTF8 -Append
        }
    }
    catch {
        $timestamp2 = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
        "[$timestamp2] [ERROR] Restart error: $($_.Exception.Message)" | Out-File $logFile -Encoding UTF8 -Append
    }
}
catch {
    $errorMsg = $_.Exception.Message
    $timestamp3 = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
    "[$timestamp3] [ERROR] Restart error: $errorMsg" | Out-File $logFile -Encoding UTF8 -Append
}
finally {
