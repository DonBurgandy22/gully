...
try {
    ...
    $sessionFile = Get-ChildItem $sessionPath -Filter "*.jsonl" -ErrorAction SilentlyContinue |
    ...
    if ($fileSizeKB -ge $threshold80KB) {
        ...
        Start-Process powershell -ArgumentList "-NonInteractive -WindowStyle Hidden -File C:\Dev\burgundy-self-restart.ps1"
    }
    ...
    Add-Content $logFile "[$timestamp] Context monitor error"
    exit 0
}