...
foreach ($module in $modules) {
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            "[$timestamp] [VERIFY]   [OK] Module: $module" | Out-File $logFile -Encoding UTF8 -Append
        } else {
            "[$timestamp] [VERIFY]   [ERROR] Module missing: $module" | Out-File $logFile -Encoding UTF8 -Append
        }
    } catch {
        "[$timestamp] [VERIFY]   [ERROR] Module check failed for $module" | Out-File $logFile -Encoding UTF8 -Append
    }
}
