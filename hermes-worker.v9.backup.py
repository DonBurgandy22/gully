...
def append_log(task_id, task_type, status, target, verified, attempt, max_retries, is_retry, current_task_file):
    timestamp = now_iso()
    line = (
        f"{timestamp} | {task_id} | {task_type} | {status} | "
        f"{target} | verified={verified} | attempt={attempt}/{max_retries} | "
        f"is_retry={is_retry} | file={current_task_file}"
    )
...
def handle_execute(task):
    ...
    if target_path.suffix.lower() != ".py":
        return "blocked", target, False, None, "Only .py execution is allowed in v9", None, "policy"
...
def process_task_file(task_file):
    ...
    max_retries = 0
    requeued = False
    source_task_file = task_file.name
    current_task_file = task_file.name
    is_retry = ".retry-" in task_file.name
    ...
    append_log(task_id, task_type, status, target, verified, attempt, max_retries, is_retry, current_task_file)
