...
try {
    ...
    # FIRST: Call complete memory save script to save ALL MD files
    Add-Content $logFile "[$timestamp] Calling complete memory save..."
    $memorySaveScript = "$workspace\burgundy-complete-memory-save.ps1"
    if (Test-Path $memorySaveScript) {
        try {
            & powershell.exe -ExecutionPolicy Bypass -NonInteractive -WindowStyle Hidden -File $memorySaveScript
            Add-Content $logFile "[$timestamp] Complete memory save executed successfully"
        }
        catch {
            Add-Content $logFile "[$timestamp] Memory save failed: $($_.Exception.Message)"
        }
    } else {
        Add-Content $logFile "[$timestamp] WARN: Memory save script not found at $memorySaveScript"
    }
    ...
    $memoryEntry = "`n## Auto-Restart — $timestamp`n- Context threshold triggered restart`n- Complete memory save executed first`n- Session cleared`n- Read session-summary.md on return`n"
    ...
    Add-Content $logFile "[$timestamp2] Sending auto-wake event..."
    Start-Sleep -Seconds 5  # Wait for gateway to be ready
    try {
        & $openclaw cron wake --text "Gateway restarted - resuming operation" --mode now
        Add-Content $logFile "[$timestamp2] Auto-wake event sent successfully"
    }
    catch {
        Add-Content $logFile "[$timestamp2] Auto-wake failed: $($_.Exception.Message)"
    }
}
catch {
